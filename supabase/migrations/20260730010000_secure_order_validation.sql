-- Trusted order creation for the customer storefront.
-- The Edge Function authenticates the request, then calls this function so
-- inventory validation and reservation happen in one database transaction.
...
begin
  ...
  -- Aggregate duplicate variant IDs before checking stock so a manipulated
  -- payload cannot bypass inventory limits by repeating the same variant.
  for requested in
  ...
  loop
    ...
    -- FOR UPDATE prevents another order from consuming this stock while the
    -- current order is being validated and created.
    select
      pv.id,
      pv.product_id,
  ...
  -- Reserve inventory when the order request is accepted. The same locked
  -- rows were validated above, so this cannot oversell under concurrency.
  update public.product_variants pv
  set stock_quantity = pv.stock_quantity - requested.quantity,
      updated_at = now()
